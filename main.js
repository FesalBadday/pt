...
// Function to play prayer sound using Web Audio API
function playPrayerSound() {
  const audioContext = new (window.AudioContext || window.webkitAudioContext)();
  const audioElement = document.createElement('audio');
  audioElement.src = 'prayer_sound.mp3'; // Add your prayer sound file
...
function showNotificationRequest() {
  const confirmed = confirm("هل ترغب في تفعيل الإشعارات لأوقات الصلاة؟");
  if (confirmed) {
    localStorage.setItem('notificationRequested', 'true');
    if ("Notification" in window) {
      Notification.requestPermission().then(permission => {
        if (permission === "granted") {
          displayNotification("Notification permission granted");
        }
      });
    }
  } else {
    localStorage.setItem('notificationRequested', 'false');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (notificationRequested !== 'true' && notificationRequested !== 'false') {
